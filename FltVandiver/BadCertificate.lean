...
/-!
# A Bad-set form of Washington Lemma 9.7

The size bound `ℓ < p² − p` enters the 9.5 descent only through Lemmas 9.6 and 9.7. Lemma 9.6
needs only `p ∤ (ℓ − 1)/p` (`JointAuxiliary`). Lemma 9.7's binomial-sum cancellation is the
special case `k < p − 1` of a finite-field statement: the set

  `Bad(p, ℓ) = { w ≠ 0 : (1 − w μ^a)^k = (1 − w μ^{−a})^k for all a }`,  `k = (ℓ−1)/p`,

is empty. `lemma_9_7_dvd_z_bad` replaces `hsize` by `p ∤ k` and `Bad = ∅`; the rest of the
descent (`caseII_95`) already takes `ℓ ∣ z` and carries no size bound. Three interfaces follow:

* `fermatLastTheoremFor_of_bad_cert`: the computable certificate `noBadCert` (an `O(ℓ)` scan
  with early exit) together with the `Q_i` certificate and a separate Sophie Germain prime;
* `noBad_of_two_mul_le`: `Bad(p, ℓ) = ∅` is proved outright for even `k ≤ (3p − 5)/2`, on the
  standard axioms, extending Washington's `k < p − 1`;
* `fermatLastTheoremFor_of_size_cert`: the frozen interface `fermatLastTheoremFor_of_certs_95`
  with its budget `ℓ < p² − p` widened to `2ℓ ≤ 3p² − 5p + 2`, no `Bad` computation needed.

The factorization plumbing is adapted from `CaseII95Descent`. No `sorry`, no new axiom, no
`native_decide`; the `p = 37`, `ℓ = 1777` examples live in `flt-vandiver-primes`
(`FltPrimes/FLT37Beyond.lean`).
...
namespace FltVandiver.BadCert

open CyclotomicNT FltVandiver FltVandiver.QiCert FltVandiver.Descent92
open FltVandiver.Descent95 FltVandiver.JointAux
